...
### Tweets vs. alcance with influencers
...
### Tweets vs. alcance with influencers zoom
...
```{r frecuency_mention, message=FALSE, warning=FALSE}
mentions_frequency<- tweets_meta %>%
    mutate(mention = str_extract(texto, "@\\w+"))  # Extraer usuarios

p <- draw_col_frequency(mentions_frequency, "mention", "total",   min_date, max_date, "mentions")
print(p)

# Salvamos la gráfica en un archivo
ggsave(file.path(image_path,paste0(params$prefix,"_users_mention_cloud.png")))
...
### Accumulated topics con amplificación
...
### Accumulated emojis con amplificación
